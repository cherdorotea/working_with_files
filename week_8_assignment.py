...
def main():
    ...
    if os.path.isdir(directory): # this checks if the path is indeed in the directory
        data = open(os.path.join(directory,filename),'w') # creating and opening the file to write
        data.write(name+", "+address+", "+phone_number+'\n') # this is the user's input, separated by a comma
        data.close() # the file will close after the writing is done
        ...
        print("Sorry. Path not located. Please try again.")
main()